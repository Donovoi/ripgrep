...
fn main() {
    ...
    for (size, label) in &sizes {
        ...
        let single =
            benchmark_decompression(&compressed, input.len(), 1, "1 ");

        if recommended >= 2 {
            let dual =
                benchmark_decompression(&compressed, input.len(), 2, "2 ");
        ...
        if recommended >= 4 {
            let quad =
                benchmark_decompression(&compressed, input.len(), 4, "4 ");
        ...
        if recommended >= 8 {
            let oct =
                benchmark_decompression(&compressed, input.len(), 8, "8 ");
    ...
    println!("\nKey Findings:");
    println!(
        "  - Small files benefit less from parallelism (thread overhead)"
    );
