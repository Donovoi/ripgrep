...
fn test_gdeflate_search() {
    ...
    let compressed = gdeflate::compress(content, 6, 0).expect("compression failed");
    ...
    file.write_all(&(content.len() as u64).to_le_bytes()).expect("failed to write size");
    ...
    assert!(output.contains("Sherlock Holmes"), "output should contain 'Sherlock Holmes'");
}
...
fn test_gdeflate_with_gz_extension() {
    ...
    assert!(output.contains("GDeflate test"), "output should contain 'GDeflate test'");
}
