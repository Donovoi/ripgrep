...
impl GDeflateReader {
    ...
    pub fn new<R: Read>(mut reader: R) -> io::Result<Self> {
        ...
        let output_size = u64::from_le_bytes(
            header[4..12].try_into()
                .map_err(|_| io::Error::new(io::ErrorKind::InvalidData, "Invalid size field"))?
        ) as usize;
        ...
        let decompressed = gdeflate_stub::decompress(&compressed, output_size, 0)
...
fn detect_decompression_strategy(path: &Path) -> io::Result<DecompressionStrategy> {
        ...
        Ok(_) => {
            ...
            if let Some(ext) = path.extension() {
                ...
                match ext_str.as_ref() {
                    "gz" | "tgz" => return Ok(DecompressionStrategy::External("gzip".to_string())),
                    "bz2" | "tbz2" => return Ok(DecompressionStrategy::External("bzip2".to_string())),
                    "xz" | "txz" => return Ok(DecompressionStrategy::External("xz".to_string())),
                    "zst" | "zstd" => return Ok(DecompressionStrategy::External("zstd".to_string())),
                    _ => {}
                }
            }
...
pub fn open_with_decompression(path: &Path) -> io::Result<UnifiedDecompressionReader> {
    ...
    match strategy {
        ...
        DecompressionStrategy::External(cmd) => {
            ...
            eprintln!("External decompression with {} not implemented in example", cmd);
            let file = File::open(path)?;
            Ok(UnifiedDecompressionReader::Direct(BufReader::new(file)))
        },
    }
...
impl DecompressionBenchmark {
    pub fn throughput_mbs(&self) -> f64 {
        (self.decompressed_size as f64 / 1024.0 / 1024.0) / (self.duration_ms as f64 / 1000.0)
    }
...
fn main() {
    ...
    println!("This example demonstrates how GDeflate would integrate with ripgrep:");
    println!();
    ...
    println!("  rg 'pattern' file.txt.gdz    # Searches GDeflate compressed file");
